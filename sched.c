...
void fairmix(struct task *list, char* current_output_file, int time_slice, int overhead){
    ...
    while (all_tasks_finished == 0){
        ...
        while (rover != NULL)
        {
            if ((is_task_finished || current_cycle % time_slice == 0) &&
                (!is_in_ready_queue(ready_queue, list_rover) && (list_rover->arrival_time <= time_count))){
                current_cycle = 0;
                if (rover->next == NULL){
                    break;
                }
        ...
        if (ready_queue != NULL && !task_run_on_current_cycle){
            ...
            int cpu_id = ready_queue->task_id;
            int serv_display = ready_queue->remaining_time;
            int nextRem = serv_display - 1;

            printf("%-5d %-6d %-12d ", time_count, cpu_id, serv_display);
            if (fp) fprintf(fp, "%-5d %-6d %-12d ", time_count, cpu_id, serv_display);
            if (nextRem <= 0) {
                printf("%-12s ", "0 (done)");
            ...
            // ready queue column excludes the running head
            struct task *rq_for_print = ready_queue->next;
            print_two_queues(fp, rq_for_print, NULL);
            printf("\n");
            if (fp) fprintf(fp, "\n");

            // mutate after printing
            ...
            // idle line in project layout
            printf("%-5d %-6s %-12s %-12s ", time_count, "", "", "");
            if (fp) fprintf(fp, "%-5d %-6s %-12s %-12s ", time_count, "", "", "");
            print_two_queues(fp, ready_queue, NULL);
            printf("\n");
            if (fp) fprintf(fp, "\n");
        }
    ...
    if (fp) fprintf(fp, "All tasks complete.\n");
    if (fp) fclose(fp);
}
...
struct task *sort_return_queue(struct task *queue, int max_min){
    ...
    // (removed "after sorting" debug printing)
